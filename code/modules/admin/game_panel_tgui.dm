...
/datum/game_panel/ui_act(action, params)
	...
	switch(action)
		...
		if("gamemode_panel")
			usr << browse(null, "window=game_panel")
			usr.client << link("?src=[ref];gamemode_panel=1")
		if("create_object")
			usr << browse(null, "window=game_panel")
			usr.client << link("?src=[ref];create_object=1")
		if("quick_create_object")
			usr << browse(null, "window=game_panel")
			usr.client << link("?src=[ref];quick_create_object=1")
		if("create_turf")
			usr << browse(null, "window=game_panel")
			usr.client << link("?src=[ref];create_turf=1")
		if("create_mob")
			usr << browse(null, "window=game_panel")
			usr.client << link("?src=[ref];create_mob=1")
